...
function lYearDays(year) {
    ...
    for (var bit = 0x8000; bit > 0x8; bit >>= 1) {
        days += (lunarInfo[year - 1900] & bit) ? 1 : 0
    }
    return days + leapDays(year)
}

function ganzhiYear(year) {
    var stem = heavenlyStems[(year - 4) % 10]
    var branch = earthlyBranches[(year - 4) % 12]
    return stem + branch
}

function formatLunarMonth(month, isLeap) {
    var name = lunarMonthNames[month - 1] + "月"
...
function solarToLunar(date) {
    ...
    var offset = Math.floor((targetDate.getTime() - baseDate.getTime()) / 86400000)
    var year = 1900
    var month = 1
    var day = 1
    var isLeap = false
    ...
    return {
        ...
        animal: zodiacAnimals[(year - 4) % 12],
        monthText: formatLunarMonth(month, isLeap),
        dayText: formatLunarDay(day),
        shortText: day === 1 ? formatLunarMonth(month, isLeap) : formatLunarDay(day),
        fullText: ganzhiYear(year) + "年(" + zodiacAnimals[(year - 4) % 12] + ") " + formatLunarMonth(month, isLeap) + formatLunarDay(day)
    }
}
